 ...
 */
void debug_print_ST(ref SuffixTree st)
{
	foreach(i, n; st.tree)
	{
		try
		{
			if (n.end == int.max) 
				writefln("%s %s %s %s %s %s", i, n.next, n.start, n.end, st.text[n.start .. st.text.length], n.pos);
			else
				writefln("%s %s %s %s %s %s", i, n.next, n.start, n.end, st.text[n.start .. n.end], n.pos);
		}
...
int[] search_ST(ref SuffixTree st, string s)
{
	...
	debug writeln("Starting search for ", s);

	if (s[0] in st.tree[0].next)
	{
		matched_characters++;
		next_node = st.tree[0].next[s[0]];
		debug writefln("An edge beginning with '%s' exists " "out of the root node pointing to node %s", s[0], st.tree[0].next[s[0]]);
		debug writefln("Matched %s character '%s'", matched_characters, s[0]);

		if (search_node(st, next_node, s, matched_characters, positions))
			return positions;
	}

	debug writeln("No match found for ", s);
	positions.length = 0;
	return positions;
}
...
{
	debug writefln("Node %s has %s children", cur_node, st.tree[cur_node].next.length);
	int[] positions;
	if (st.tree[cur_node].next.length == 0)
	{
		debug writeln("Node ", cur_node, " has no children");
		positions ~= st.tree[cur_node].pos;
		return positions;
	}
	foreach (n; st.tree[cur_node].next)
	{
		debug writeln("Checking node ", n, " for outgoing edges");
...
bool search_node(ref SuffixTree st, int cur_node, string s, ref int matched_characters, ref int[] positions)
{
	 ...
	 * previous node. 
	 */
	debug bool checked_edge;
	foreach (pos; st.tree[cur_node].start+1 .. st.tree[cur_node].end)
	{
		debug checked_edge = true;
		debug writeln("Comparing with characters on edge of node ", cur_node);

		if (s[matched_characters] == st.text[pos])
		{
			debug writeln("Matched ", s[matched_characters]);
			matched_characters++;

			// Don't continue searching if complete query string is matched now.
			if (matched_characters >= s.length)
			{
				debug writeln("FOUND COMPLETE MATCH");
		...
		{
			debug writeln("No complete match!");
			return false;
		}
		
	}

	debug if (!checked_edge) { writeln("Skipped checking characters on edge of node ", cur_node, ". Edge too short!");};
	...
	{
		debug writefln("An edge beginning with '%s' exists out of node %s pointing to %s",
					   s[matched_characters], cur_node, st.tree[cur_node].next[s[matched_characters]]);
		debug writeln("Matched ", s[matched_characters]);

		// Don't continue searching if complete query string is matched now.
		if (matched_characters == s.length)
		{
			debug writeln("FOUND COMPLETE MATCH ON OUTGOING EDGE");
